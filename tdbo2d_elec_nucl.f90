...
! parameter
  ...
  real(8),parameter :: r_dist = 2.59d0
...
! wfn(0,:) == wfn(Nx,:) == 0 
  ...
  do ix = 1,Nx-1  
    ...
    wfn_e(ix) = r !exp(-0.5d0*(xn(ix)-1.0)**2)
  end do
  do ix = 1,NR-1
!    call random_number(r); r = r-0.5d0
...
!     v_en(iy,ix) = -1d0/sqrt(1d0+rp**2)-1d0/sqrt(1d0+rm**2)
!     v_ne(ix,iy) = -1d0/sqrt(1d0+rp**2)-1d0/sqrt(1d0+rm**2)
     v_e(ix) = -1d0/sqrt(1d0+rp**2)-1d0/sqrt(1d0+rm**2)+1d0/sqrt(0.03d0+r**2)
  end do
...
!    res_n = sum( (hwfn_n - esp_n*wfn_n)**2)*dr
    ...
    write(30,"(I7,2x,999e16.6e3)")iter,Etot,esp_e,res_e
    write(*,"(I7,2x,999e16.6e3)")iter,Etot,esp_e,res_e
